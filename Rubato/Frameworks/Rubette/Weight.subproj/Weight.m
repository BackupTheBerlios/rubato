...
+ (void)initialize 
{
	...
	[Weight setVersion:6]; // from 6 is Apple
    }
    return;
}
...
- (NSArray *)attributeKeys;
{
    ...
    if (!keys)
        keys=[[NSArray alloc] initWithObjects:@"name",@"lowNorm",@"highNorm",@"range",@"tolerance",@"isInverted",@"meanNormalizedWeight",@"mySpace",nil]; // hope mySpace works as int.
/* or as variable:
    double myMinWeight;
    double myMaxWeight;
    double myMeanWeight;
...
- (NSArray *)toOneRelationshipKeys;
{
    static NSArray *keys=nil;
    if (!keys)
        keys=[[NSArray alloc] initWithObjects:@"myParameterTable",@"parameterObject",@"myBDWeight",nil];
    return keys;
}
- (NSArray *)toManyRelationshipKeys;
{
    static NSArray *keys=nil;
    if (!keys)
        keys=[[NSArray alloc] initWithObjects:@"myWeightEvents",@"myParameterObjectBundles",nil];
    return keys;
}
...
//    const void *aKey, *aVal;    
    ...
    Weight *myCopy = JGSHALLOWCOPY;
    myCopy->myWeightEvents = [myWeightEvents mutableCopyWithZone:zone];//ref];
...
//    }
    ...
    myCopy->myConverter = [[StringConverter alloc]init];

    [myCopy setParameterObject:myParameterObject requiredBundles:[myParameterObjectBundles mutableCopyWithZone:zone]];
    return myCopy;
}
...
//    [super initWithCoder:aDecoder];
    if (classVersion<5)
        NSLog(@"Trying to load Weight with Version<5. Please resave on Nextstep with newer Rubato version.");
...
//    myRefCount = INIT_REFCOUNT;
    if (!myConverter) myConverter = [[StringConverter alloc]init];
...
// new:
	id oldVal, key, val;
	key= [NSString jgStringWithCString:paraName];// autorelease]; (in included in constructor).
...
{
/* jg ist glaub ich ueberfluessig, weil ja NSDictionary sowieso keine doppelten Eintraege hat. siehe auch Preferences.m
...
{
   // jg changed the order of the following two lines 
...
{
    id aString = [[StringConverter alloc]init], stringList, sortList;
//    char *data;
    int i, c;//, len, maxlen;
    NSMutableString *privateMutableString = [NSMutableString new];
    
// was:    [myParameterTable appendToString:privateMutableString]; // jg: Dictionary description method
    [privateMutableString appendString:[myParameterTable description]]; // jg is
    
    [aString setStringValue:privateMutableString];
...
//	if (i<c-1) NXPutc(aCoder, '\n');
    ...
    [sortList release];
//    JGCloseMemory(privateMutableString, NX_FREEBUFFER);
    return self;
}
...
//    [localArchiver encodeRootObject:myParameterObject];
  ...
  if (myParameterObject)
    localArchive=[[aCoder class] archivedDataWithRootObject:myParameterObject];
  else
    localArchive=nil;
//    [aCoder encodeValueOfObjCType:"i" at:&length];
//    for (i=0; i<length; i++)
//	[aCoder encodeValueOfObjCType:"c" at:&buffer[i]];
    [aCoder encodeObject:localArchive];
...
- oldReadParameterObject:(NSCoder *)aDecoder;
{
//    id returnValue;
    char bundles[1025];
    char *buffer;
    ...
    [self needsBundles:bundles];
    data=[NSData dataWithBytesNoCopy:buffer length:length];
	
    NS_DURING
    myParameterObject=[[aDecoder class] unarchiveObjectWithData:data]; // localArchiver
    NS_HANDLER
//jg    if (NXLocalHandler.code==NSArchiverClassError && strlen(bundles)) {
...
//jg	LOAD_HANDLER  /* a load handler macro in macros.h */
    ...
    // free(buffer); // uncommented, because maybe freed by autoreleased data. Anyhow, old code so dont care about memory leak. 
    return self;
}
...
//    int i, length;
  unsigned oldVersion=[aDecoder versionForClassName:@"Weight"];
 
   if (oldVersion<5) {
     NSRunAlertPanel(@"Read Weight",@"Weight previous to version 5. Please reload and save with Rubato 1.0 on NextStep.",@"",nil,nil);
     return self;
   } 
   if (oldVersion==5) {
     [self oldReadParameterObject:aDecoder];
     return self;
   }
...
////    myParameterObject = NXReadObjectFromBufferWithZone(buffer, length, [self zone]);
      if (buffer)
        myParameterObject=[[[aDecoder class] unarchiveObjectWithData:buffer] retain]; // replaced JG... at 22.06.2001 by jg
      else
        myParameterObject=nil;
    NS_HANDLER
//jg    if (NXLocalHandler.code==NSArchiverClassError && strlen(bundles)) {
	NSRunAlertPanel(@"Read Weight", @"The following Modules must be loaded in order to read all weight parameters: %s", @"", nil, nil, bundles);
	NS_VALUERETURN(nil, id); // war statt id: typeof(nil));
//jg    } else
//jg	LOAD_HANDLER  /* a load handler macro in macros.h */
    NS_ENDHANDLER /* end of handler */
    if ([myParameterObject conformsToProtocol:@protocol(RefCounting)])
...
- bDWeightTo:(int)index;
{
	...
	while([bdWeightEvents count]) {
            bdEvent = [[bdWeightEvents objectAt:0] retain];
            [bdWeightEvents removeObjectAt:0];
	
            bdEventAti=[[bdWeightEvents objectAt:0] retain];
...
- (double)bDSplineTo:(int)index at:anEvent;
{
	...
	if (![myBDWeight isSuperspaceFor:spaceOfIndex(index)]) {
	    ...
	    myBDWeight = [[self bDWeightTo:index] retain]; // jg:retain
	}
